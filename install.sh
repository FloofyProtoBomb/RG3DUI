...
if [[ $(uname -o) == "Android" ]]; then
    ...
    if grep -q "Raspberry" /proc/device-tree/model || grep -q "Orange" /proc/device-tree/model || grep -q "Rockchip" /proc/device-tree/model || lscpu | grep -q "ARM" || lscpu | grep -q "aarch64"; then
        ...
        # Update and install necessary packages
        run_command sudo apt-get update
        run_command sudo apt-get install -y git bc libcurl4-openssl-dev libssl-dev libjansson-dev automake autotools-dev build-essential cron screen netcat-openbsd jq iproute2 gawk
